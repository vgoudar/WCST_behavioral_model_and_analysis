...
class StateInputDrivenTransitions(InputDrivenTransitions):
    """
    Hidden Markov Model whose transition probabilities are
    determined by a generalized linear model applied to the
    exogenous input.
    """
    def __init__(self, K, D, M, alpha=1, kappa=0, l2_penalty=0.0):
        super(StateInputDrivenTransitions, self).__init__(K, D, M=M, alpha=alpha, kappa=kappa)
    ...
    def permute(self, perm):
        ...
        """
        self.log_Ps = self.log_Ps[np.ix_(perm, perm)]
        self.Ws = self.Ws[np.ix_(perm, perm)]
    ...
    def log_transition_matrices(self, data, input, mask, tag):
        ...
        log_Ps = np.tile(self.log_Ps[None, :, :], (T-1, 1, 1))
        # Input effect
        A = np.reshape(self.Ws,[self.K*self.K,self.M])
        A = np.dot(input[1:],A.T)
        A = np.reshape(A,[T-1,self.K,self.K])
        log_Ps += A
        return log_Ps - logsumexp(log_Ps, axis=2, keepdims=True)
