...
def forward_pass2(pi0,
    ...
    hetero = (Ps.shape[0] == T-1)
    alphas[0] = np.log(pi0) + log_likes[0]
    alphas2[0] = np.log(pi0)
    # print('up2')
    for t in range(T-1):
        m = np.max(alphas[t])
        # m2 = np.max(alphas2[t])
        alphas[t+1] = np.log(np.dot(np.exp(alphas[t] - m), Ps[t * hetero])) + m + log_likes[t+1]
        alphas2[t+1] = np.log(np.dot(np.exp(alphas[t] - m), Ps[t * hetero])) + m
    return logsumexp(alphas[T-1])
...
def hmm_expected_states2(pi0, Ps, ll):
    T, K = ll.shape

    alphas = np.zeros((T, K))
    alphas2 = np.zeros((T, K))
    forward_pass2(pi0, Ps, ll, alphas, alphas2)
    normalizer = logsumexp(alphas[-1])
